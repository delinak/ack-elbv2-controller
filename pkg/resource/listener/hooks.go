...
func (rm *resourceManager) updateTags(
	...
	defer func() { exit(err) }()
	return tags.SyncRecourseTags(ctx, rm.sdkapi, rm.metrics, string(*desired.ko.Status.ACKResourceMetadata.ARN), latest.ko.Spec.Tags, desired.ko.Spec.Tags, convertToOrderedACKTags, fromACKTags)
}